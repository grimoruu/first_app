...
def get_all_tasks_api(
    ...
    return get_tasks_service(task=task, user_id=user_id, db=db)
...
def create_task_api(
    ...
    return create_task_services(task=task, user_id=user_id, db=db)
...
def update_task_api(
    ...
    return update_task_services(task=task, user_id=user_id, db=db)


@router.delete("", status_code=status.HTTP_200_OK, response_model=TaskResponse)
def delete_task_api(
    task: TaskDeleteSchema,
    user_id: int = Depends(get_user_by_token),
    db: Session = Depends(get_db),
) -> TaskResponse:
    return delete_task_services(task=task, user_id=user_id, db=db)
