...
def create_board_api(
    ...
    return create_board_services(board=board, user_id=user_id, db=db)
...
def update_board_api(
    ...
    return update_board_services(board=board, user_id=user_id, db=db)


@router.delete("", status_code=status.HTTP_200_OK, response_model=BoardResponse)
def delete_board_api(
    board: BoardDeleteSchema,
    user_id: int = Depends(get_user_by_token),
    db: Session = Depends(get_db),
) -> BoardResponse:
    return delete_board_services(board=board, user_id=user_id, db=db)
